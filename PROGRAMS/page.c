...
int main(int argc, char ** argv)
{
	...
	strcpy (outqfile, expfile);
	strcat (outqfile, "_PAGE/input.ipage_quantized");

	strcpy (outifile, expfile);
	strcat (outifile, "_PAGE/output.ipage_intersections");
	//
	...
	printf("Creating hash table for gene names in gene ontology index file ... ");
	...
	//
	printf("Populating the GO profile table ... ") ;
	...
	printf("Writing internal quantized groups ...") ;
	file_outqfile = fopen(outqfile, "wt");
	fprintf(file_outqfile,"Identifier\tCluster\n");
	for(ii = 0; ii < gene_num; ii++) {
	  fprintf(file_outqfile, "%s\t%d\n",E_names[ii],E_q[ii]);
	}
	fclose(file_outqfile);
	printf("Done\n") ;
	...
	for (i=0 ; i<nb_cat ; i++){
		...
		for (j=0 ; j<cluster_num ; j++){
			double po = log10(cumhyper  (cluster_go_count[j], cluster_gene_count[j], cat_gene_count, gene_num)) ;
			double pu = log10(cumhyper_u  (cluster_go_count[j], cluster_gene_count[j], cat_gene_count, gene_num)) ;
			if (po<pu){
				pvalue  [j][i] = -1*po ;
			}else{
				pvalue  [j][i] = pu ;
	...
	// output intersection
	file_outifile = fopen(outifile, "wt");
	fprintf(file_outifile,"Group\tCluster\tIdentifiers\n");
	for (jj = 0; jj< nb_cat; jj++) {
		int go_index1 = A_go_indices[jj] ;
		M_q = c_matrix_column_binarized(go_profile, go_index1, gene_num);
		int kk;
		for (kk = 0; kk < cluster_num; kk++) {
			fprintf(file_outifile, "%s %s", go_terms_list[ A_go_indices[jj] ],  go_desc[A_go_indices[jj]]) ;
			fprintf(file_outifile, "\t%d", kk);
			for (j=0 ; j<gene_num ; j++){
				if (M_q[j]==1 && E_q[j] == kk){
					fprintf(file_outifile, "\t%s", E_names[j]);
				}
			}
			fprintf(file_outifile,"\n");
		}
		free(M_q);
	}
	fclose(file_outifile);
	// end output intersection
	...
	//
	printf("Writing the p-value matrix to file...") ;
