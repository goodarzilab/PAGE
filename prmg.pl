...
# protein add
# should never be used as prmg_protein exists
if (defined $species && $protein == 2 && ($species eq 'human' || $species eq 'mouse')) {
	$species .= 'p';
}
# protein end
...
if ( ( $expfile =~ /\*/ ) or ( $submit == 1 ) ) {
	...
	foreach my $file (@$files) {
		...
		$firedirsymbol = 'DNA_RNA';
		if (!(-e $firefile)) {
			$firefile = $file . "_FIRE/DNA/$f.summary";
			$firedirsymbol = 'DNA';
		}
		if (!(-e $firefile)) {
			$firefile = $file . "_FIRE/RNA/$f.summary";
			$firedirsymbol = 'RNA';
		}
		if (!(-e $firefile)) {
			die "FIRE summary file does not exist or cannot be opened.\n";			
		}
		my $pagefile = $file . "_PAGE/pvmatrix.txt";

		my $cmd =
"perl $pagedir/prmg.pl --expfile=$file --firefile=$firefile --pagefile=$pagefile --goindexfile=$goindexfile --max_p=$max_p --dorna=$dorna --species=$species --homologiesfile=$homologiesfile --genelist=$genelist --nodups=$nodups";
		$pbs->addCmd($cmd);

		my $generatehtmlcmd =
		  "perl $pagedir/SCRIPTS/motif_cat_draw_html.pl --expfile=$file";
...
if ($firefile =~ m/DNA_RNA\/$fn\.summary$/) {
	$firedirsymbol = 'DNA_RNA';
} elsif ($firefile =~ m/DNA\/$fn\.summary$/) {
	$firedirsymbol = 'DNA';
} elsif($firefile =~ m/RNA\/$fn\.summary$/) {
	$firedirsymbol = 'RNA';	
}
if($dorna == 1) {
  if (-e $firefile) {
		 ...
		 my $fn = Sets::filename($expfile);
     $firefile = "$expfile\_FIRE/RNA/$fn.summary";
     $firedirsymbol = 'RNA';
  }
}
...
my $firebinfile = substr( $firefile, 0, rindex( $firefile, $firedirsymbol) );
my $filename = Sets::filename($firefile);
$filename =~ s/\.summary/\.profiles/;
my %motif_gene;
my %gene_motif;
print $firedirsymbol;
if ($firedirsymbol eq 'DNA_RNA' || $firedirsymbol eq 'DNA') {
...
if ($firedirsymbol eq 'DNA_RNA' || $firedirsymbol eq 'RNA') {
...
if ($firedirsymbol eq 'DNA_RNA' || $firedirsymbol eq 'DNA') {	
	...
	$filename2 =~ s/profiles$/motifnames/;
	$ta->loadFile("$firebinfile/DNA/$filename2");
...
if ($firedirsymbol eq 'DNA_RNA' || $firedirsymbol eq 'RNA') {	
	if ( $dorna == 1 ) {
		...
		my $filename2 = $filename;
		$filename2 =~ s/profiles$/motifnames/;
		$ta->loadFile("$firebinfile/RNA/$filename2");
		my $a_motif_name = $ta->getArray();
		foreach my $r (@$a_motif_name) {
			my $motif = $r->[0];
			my $name  = $r->[1];
...
sub printHeatmapBars {
	...
	for ( my $j = 0 ; $j < @$a_ta_H ; $j++ ) {
		if ( $header_motif eq "true" ) {
			...
			close OUT;
			system(
"$pagedir/SCRIPTS/weblogo/seqlogo -f ./Temp/$motif.txt -F EPS  -a -c -M -n -Y -w 5 -h 3 > ./Temp/$motif.eps"
			);
